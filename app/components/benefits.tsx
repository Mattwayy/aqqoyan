      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-8 md:gap-12 px-4">
        ...
        <div className="w-full grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 lg:gap-8">
          {audienceData.map((item, index) => (
            <div
              key={index}
              className="group flex flex-col items-center gap-2 sm:gap-4 rounded-2xl border border-slate-100 p-4 sm:p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-[#2f4fa3]/30"
            >
              <div className="flex h-10 w-10 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-[#121e52]/5">
                <Image
                  src={item.icon}
                  alt={item.title}
                  width={40}
              ...
              <h3 className="text-xs sm:text-base md:text-xl font-semibold text-slate-800 group-hover:text-[#2f4fa3] transition-colors text-center leading-snug">
                {item.title}
              </h3>
              <p className="text-slate-600 text-center text-[11px] sm:text-sm leading-snug hidden sm:block">
                {item.description}
              </p>
            </div>
          ))}
