      ...
      <div className="mx-auto px-4 max-w-6xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 lg:gap-8">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="group bg-white rounded-xl p-4 sm:p-6 shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-[#2f4fa3] hover:-translate-y-1 flex flex-col items-center text-center"
            >
              <h3 className="text-sm sm:text-base md:text-xl font-semibold text-gray-800 mb-1 sm:mb-2">
                {benefit.title}
              </h3>
              <p className="text-gray-600 leading-relaxed text-[11px] sm:text-sm hidden sm:block">
                {benefit.description}
              </p>
            </div>
          ))}
